...
def page_cover(pdf, page, total):
    ...
    fig.text(0.5, 0.62,
             '—— 基于您当前的青训工作流,系统能力如何放大您的教学 ——',
             ha='center', fontsize=11, color=MUTED, style='italic')

    callout(fig, 0.15, 0.36, 0.70, 0.18, kind='warning')
    fig.text(0.5, 0.515, '● 关于本预览', ha='center',
             fontsize=12, color=WARNING, weight='bold')
    notes = [
        '• 文中所有数字、姓名、图表均为演示 mock 数据,非真实球员',
        '• 您已经见过的"中青赛_1"基础报告是当前真实输出',
        '• 本预览中"完整球员档案、训练前后对比、长期发展轨迹"',
        '  对应 1-3 个月内可演进出的能力(末页有诚实清单)',
        '• 目的:让您评估"如果工具长成这样,是否能放大您的教学"',
    ]
    for i, t in enumerate(notes):
        fig.text(0.18, 0.49 - i * 0.022, t, fontsize=10, color=INK)
...
def page_positioning(pdf, page, total):
    ...
    is_items = [
        '• 一个本地化运行的视频分析工具,数据存在您自己的设备上',
        '• 把场上发生的事情精确描述出来:谁、什么时间、在哪、做了什么',
        '• 为您提供"客观证据",让训练效果、球员发展可被记录和回看',
        '• 您评级答辩 / 公开课 / 比武 / 家长沟通的辅助材料生成器',
    ]
    ...
    fig.text(0.5, 0.405,
             '您原本要凭感觉、记忆、笔记完成的部分,系统替您变成可记录、',
             ha='center', fontsize=11, color=INK)
    fig.text(0.5, 0.382,
             '可查阅、可作为评级/公开课/比武答辩附件的客观数据。',
             ha='center', fontsize=11, color=INK)
    fig.text(0.5, 0.348,
             '数据怎么解读、怎么用、用到什么程度,完全由您决定。',
...
# ---------------------------------------------------------------------------
# Page 3 — Scenario 1: Training effect proof
# ---------------------------------------------------------------------------
...
def page_scenario_training_effect(pdf, page, total):
    ...
    fig.text(0.06, 0.86, '您的工作环节', fontsize=12, color=INK, weight='bold')
    work = [
        '• 您设计训练课时已经有清晰的目标(本例:提升前场压迫强度)',
        '• 您已经在比赛中观察孩子们是否学到了——靠经验和直觉',
        '• 评级答辩、公开课需要"训练前后对比"的证据,但拿不出来',
    ]
    for i, t in enumerate(work):
        fig.text(0.08, 0.835 - i * 0.022, t, fontsize=10, color=INK)

    fig.text(0.06, 0.755, '系统替您做的', fontsize=12, color=ACCENT, weight='bold')
    fig.text(0.08, 0.730,
    ...
    fig.text(0.08, 0.232, '对您的具体价值', fontsize=10.5,
             color=ACCENT, weight='bold')
    fig.text(0.08, 0.207,
             '• 公开课答辩时,这张图就是"训练目标 → 训练效果"的硬证据',
             fontsize=9.5, color=INK)
    fig.text(0.08, 0.187,
             '• 评级时常被问"您怎么知道这套训练有效?" —— 这张图回答',
             fontsize=9.5, color=INK)
    fig.text(0.08, 0.167,
...
def page_scenario_player_trend(pdf, page, total):
    ...
    fig.text(0.06, 0.755, '系统替您做的', fontsize=12,
             color=ACCENT, weight='bold')
    fig.text(0.08, 0.730,
    ...
    fig.text(0.08, 0.275, '对您的具体价值', fontsize=10.5,
             color=ACCENT, weight='bold')
    fig.text(0.08, 0.250,
...
def page_scenario_review(pdf, page, total):
    ...
    fig.text(0.06, 0.770, '系统替您做的', fontsize=12,
             color=ACCENT, weight='bold')
    fig.text(0.08, 0.745,
             '自动标出 20-30 个候选瞬间(类型 + 球员 + 时间),您只需快速勾选',
             fontsize=10, color=INK)
    ...
    fig.text(0.08, 0.255, '对您的具体价值', fontsize=10.5,
             color=ACCENT, weight='bold')
    fig.text(0.08, 0.230,
...
def page_scenario_parent_card(pdf, page, total):
    ...
    fig.text(0.08, 0.16, '对您的具体价值', fontsize=10.5,
             color=ACCENT, weight='bold')
    fig.text(0.08, 0.135,
             '• 家长群可分享,无需多解释 —— 数据自带说服力',
             fontsize=9.5, color=INK)
    fig.text(0.08, 0.115,
             '• 您的教练评语保留主观判断空间;数据只是"由头"',
             fontsize=9.5, color=INK)
    fig.text(0.08, 0.095,
...
def page_closing(pdf, page, total):
    fig = new_page()
    page_header(fig, '下一步建议',
                '如果方向对得上,我们这样推进')

    fig.text(0.06, 0.85, '如果您觉得方向有用,我们可以这样开始',
             fontsize=12, color=INK, weight='bold')

    steps = [
        ('第 1 周',
         '您选一名孩子 + 提供 1 场近期比赛视频(或之前的"中青赛_1"也行),',
         '我们生成一份"球员发展报告" demo,让您评估格式是否合适。'),
        ('第 2-3 周',
         '基于您的反馈调整模板;约定 1-2 个您下次评级 / 公开课要用的具体场景,',
         '对接做适配(训练前后对比 / 长期发展曲线)。'),
        ('第 4-6 周',
         '用您自己的 2-3 场真实比赛数据,跑通完整流水线,产出您可以直接带到',
         '答辩 / 家长会 / 公开课的成品。'),
    ]
    y = 0.78
    for label, line1, line2 in steps:
        callout(fig, 0.06, y - 0.10, 0.88, 0.10, kind='feature')
        fig.text(0.08, y - 0.012, label, fontsize=11,
                 color=ACCENT, weight='bold')
        fig.text(0.08, y - 0.038, line1, fontsize=10, color=INK)
        fig.text(0.08, y - 0.058, line2, fontsize=10, color=INK)
        y -= 0.13

    callout(fig, 0.06, 0.27, 0.88, 0.12, kind='warning')
    fig.text(0.08, 0.37, '我们不会承诺的事',
             fontsize=11, color=WARNING, weight='bold')
    fig.text(0.08, 0.343,
             '• 不替代您的判断;数据是辅助,不是决策依据',
             fontsize=9.5, color=INK)
    fig.text(0.08, 0.322,
             '• 不在没有真实数据的情况下评估球员;一切以您的视频为准',
             fontsize=9.5, color=INK)
    fig.text(0.08, 0.301,
             '• 不在没有您许可的情况下,把您的数据 / 视频共享给任何第三方',
             fontsize=9.5, color=INK)
    fig.text(0.08, 0.280,
             '• 不承诺"和体育局打通"等需要外部协调的事 —— 我们做工具',
             fontsize=9.5, color=INK)

    fig.text(0.5, 0.18, '感谢您愿意花时间评估这套工具。',
             ha='center', fontsize=11, color=INK)
    fig.text(0.5, 0.15,
             '您是青训实操专家;我们是工程师 —— 产品形态需要您来塑造。',
             ha='center', fontsize=10, color=MUTED, style='italic')
    fig.text(0.5, 0.09,
             '如有问题或反馈,可直接回复我们;您的每一条意见都会被认真处理。',
             ha='center', fontsize=9, color=MUTED)
